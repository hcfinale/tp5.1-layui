...
class ShopCart extends Base {
    ...
    public function del($id){
        if (request()->isGet()){
            $res = $this->Cart->delCartOrder($id);
            if ($res == true){
                return json(['code'=>'1001','data'=>'删除成功']);
            }else
                return json(['code'=>'1004','data'=>'删除失败']);
        }
        return json(['code'=>'1004','data'=>'非法请求']);
    }
    // 购物车商品购买
    public function pay(){
        $goodsId = input('get.did');
        $goodsSum = input('get.sum');
